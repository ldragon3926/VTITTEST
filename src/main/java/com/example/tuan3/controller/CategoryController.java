...
public class CategoryController {
    ...
    public ResponseEntity<?> view(@RequestParam(name = "page", defaultValue = "0", required = false) Integer page){
        return ResponseUltils.success(categoryService.phanTrang(page),"ROLE_VIEW_CATEGORY","Lấy danh sách phân trang thành công");
    }
    @GetMapping("/all")
    public ResponseEntity<?> getAll(){
        return ResponseUltils.success(categoryService.getAll(),"ROLE_VIEW_CATEGORY", "Lấy tất cả danh sách thể loại thành công");
    }
    ...
    public ResponseEntity<?> getOne(@PathVariable(name = "id") Long id){
        ...
        return ResponseUltils.success(categoryService.findById(id),"ROLE_VIEW_CATEGORY", "Lấy thể loại thành công");
    }
    @PostMapping("/create")
    public ResponseEntity<?> add(@RequestBody @Valid CategorySet categorySet, BindingResult bindingResult ){
        ...
        return ResponseUltils.success(categoryService.add(category), "ROLE_DELETE_CATEGORY", "Thêm thể loại thành công");
    }
    ...
    public ResponseEntity<?> update(@RequestBody @Valid CategorySet categorySet, BindingResult bindingResult, @PathVariable(name = "id") Long id ){
        ...
        Category category = categorySet.dto(new Category());
        return ResponseUltils.success(categoryService.update(category, id),"ROLE_UPDATE_CATEGORY", "Update thể loại thành công");
    }
    ...
    public ResponseEntity<?> delete( @PathVariable(name = "id") Long id ){
        ...
        return ResponseUltils.success(null,"ROLE_DELETE_CATEGORY", "Xóa category thành công");
    }
}
